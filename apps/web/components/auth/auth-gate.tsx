...
import { ArrowRight, CircleUserRound, Github, Sparkles } from "lucide-react";
...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    api.me().then(() => setSignedIn(true)).catch(() => localStorage.removeItem("nexus_token")).finally(() => setReady(true));
  }, []);
  ...
  return <main className="mesh grid min-h-screen place-items-center p-5"><section className="panel-glow w-full max-w-md rounded-[2rem] border border-border bg-panel/95 p-7"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-accent text-bg"><Sparkles /></div><p className="mt-6 text-xs font-bold uppercase tracking-[0.2em] text-accent">Nexus AI workbench</p><h1 className="mt-2 font-serif text-3xl font-bold">Your context, kept yours.</h1><p className="mt-3 text-sm leading-6 text-muted">Sign in to keep conversations, knowledge, and project work private to your account.</p><div className="mt-7 grid gap-3"><button type="button" onClick={() => window.location.assign(api.oauthStartUrl("google"))} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-border bg-white/5 px-4 py-3 text-sm font-semibold hover:border-accent/50"><CircleUserRound size={17} />Continue with Google</button><button type="button" onClick={() => window.location.assign(api.oauthStartUrl("github"))} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-border bg-white/5 px-4 py-3 text-sm font-semibold hover:border-accent/50"><Github size={17} />Continue with GitHub</button></div><div className="my-5 flex items-center gap-3 text-xs text-muted before:h-px before:flex-1 before:bg-border after:h-px after:flex-1 after:bg-border">or use email</div><form onSubmit={submit} className="space-y-3">{register && <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Your name" required className="w-full rounded-2xl border border-border bg-black/15 px-4 py-3 text-sm outline-none focus:border-accent" />}<input value={email} onChange={(event) => setEmail(event.target.value)} type="email" placeholder="you@example.com" required className="w-full rounded-2xl border border-border bg-black/15 px-4 py-3 text-sm outline-none focus:border-accent" /><input value={password} onChange={(event) => setPassword(event.target.value)} type="password" minLength={8} placeholder="Password (at least 8 characters)" required className="w-full rounded-2xl border border-border bg-black/15 px-4 py-3 text-sm outline-none focus:border-accent" />{error && <p className="text-sm text-red-300">{error}</p>}<button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-accent px-4 py-3 font-bold text-bg disabled:opacity-50">{loading ? "Please wait…" : register ? "Create my workspace" : "Sign in"}<ArrowRight size={16} /></button></form><button onClick={() => { setRegister(!register); setError(""); }} className="mt-5 text-sm text-muted underline underline-offset-4 hover:text-text">{register ? "Already have an account? Sign in" : "New here? Create an account"}</button></section></main>;
}
